<template>
  <div class="page-sandbox">
    <template v-if="routes.length">
      <h3>Select a sandbox page from the list<template v-if="bundle.length > 1"> or go to the <router-link flat no-caps color="primary" :to="{ name: 'sandbox-bundle-routes', params: { routes: bundled ? null : bundle.join(',') } }">bundle</router-link></template>:</h3>
...
watch(bundle, () => {
  const is_bundled = bundle.value.length === routes.length
    ? true
    : !bundle.value.length
      ? false
      : undefined
